...
require ('../model/db_connection/connection.php');
   ...
   $conns = $db_con->ConnectToMYSQL();
	...
	$v_master_prod_id = $_GET['v_master_proj_id'];
...
<body>
    <div style="text-align:right; padding-right:30px; padding-top:20px;">
...
<table width="800" border="0" align="center" cellpadding="5" cellspacing="0" id="table_material_list">
  ...
  <tr>
    <td colspan="6">Project : <?php echo $project_name; ?></td>
  </tr>
  <tr>
    <td colspan="6">Product : <?php echo $product_name; ?></td>
  </tr>
  <tr>
    <td colspan="6">Total Item : <?php echo $product_qty; ?></td>
  ...
  <tr>
    ...
    ?> 
  <tr>
    <td colspan="5" class="style2" style="text-align: left;"><b>Amount</td>
	<td class="style2" style="text-align:right;"><b><?PHP echo number_format($tot_amount,3,".",",");   ?></td>
  </tr>
  <tr>
    <td colspan="5" class="style2" style="text-align: left;"><b>Single Cost Before VAT</td>
	<td class="style2" style="text-align:right;"><b><?PHP echo number_format($result,3,".",",");   ?></td>
  </tr>
</table><br>

<table width="800" border="0" align="center" cellpadding="5" cellspacing="0" id="table_material_list2">
    <tr>
        <td class="style2" style="text-align:center; color: #FFFFFF;" bgcolor="#0079DD"><strong>Mt. Cost</strong></td>
        <td class="style2" style="text-align:center; color: #FFFFFF;" bgcolor="#0079DD"><strong>VAT(%)</strong></td>
        <td class="style2" style="text-align:center; color: #FFFFFF;" bgcolor="#0079DD"><strong>VAT(%) Amt</strong></td>
        <td class="style2" style="text-align:center; color: #FFFFFF;" bgcolor="#0079DD"><strong>VAT Total</strong></td>
    </tr>
    <?php
    ...
    ?>
    <tr>
        <td class="style2" style="text-align:center;"><?php echo $row_material_list['project_item_primary_amt']; ?></td>
        <td class="style2" style="text-align:center;"><?php echo $row_material_list['vat_percentage']; ?></td>
        <td class="style2" style="text-align:center;"><?php echo $row_material_list['vat_prct_amount']; ?></td>
        <td class="style2" style="text-align:right;"><?php echo $row_material_list['vat_amount']; ?></td>
    </tr>
    <?php } ?>
    <tr>
        <td colspan="3" class="style2" style="text-align:left;"><strong>Material Cost</strong></td>
        <td class="style2" style="text-align:right;"><strong><?php echo number_format($total_amount, 3, ".", ","); ?></strong></td>
    </tr>
    <tr>
    <td colspan="3" class="style2" style="text-align: left;"><b>Single Cost After VAT</td>
	<td class="style2" style="text-align:right;"><b><?PHP echo number_format($result,3,".",",");   ?></td>
  </tr>
</table>
<br>
...
<table width="800" border="0" align="center" cellpadding="5" cellspacing="0" id="table_material_list3">
  ...
  <tr>
   ...
   <tr>
    ...
    <tr>
        <td colspan="3" class="style2" style="text-align:left;"><strong>Total Cost</strong></td>
        <td class="style2" style="text-align:right;"><strong><?php echo number_format($margin_cost_amt, 3, ".", ","); ?></strong></td>
    </tr>
    <tr>
        <td colspan="3" class="style2" style="text-align:left;"><strong>Single Cost</strong></td>
        <td class="style2" style="text-align:right;"><strong><?php echo number_format($resultt, 3, ".", ","); ?></strong></td>
    </tr>
</table>
</body>
</html>
...
<script>
    ...
    $(document).ready(function(){
        // Set initial dark color on page load
        $('input[type="button"]').css({
            'background-color': '#0056b3', // Dark blue on page load
            'color': 'white',
            'border': 'none',
            'padding': '12px 24px',
            'font-size': '16px',
        ...
        $('input[type="button"]').hover(
            ...
            function() { // Mouse leave
                $(this).css({
                    'background-color': '#0056b3',  // Dark blue when not hovered
        ...
        $('input[type="button"]').on('mousedown', function() {
            $(this).css('background-color', '#004085'); // Even darker blue on click (active state)
        }).on('mouseup', function() {
            $(this).css('background-color', '#007BFF'); // Reverts to light blue after click
        });

        $('#print_but').click(function(){
		   var v_master_prod_id = <?php echo $v_master_prod_id; ?>;
		   //var company_id = <?php echo $company_id; ?>;
		   //alert(finished_pdt_id + ' '+company_id);
		   window.open("pdf/print/quick_calculations.php?v_master_prod_id="+v_master_prod_id,"_blank");  
	    });
	}); 
	...
	function fnExcelReport() {
        ...
        // Get all tables by their IDs
        var tab1 = document.getElementById('table_material_list'); // First table
        var tab2 = document.getElementById('table_material_list2'); // Second table
        var tab3 = document.getElementById('table_material_list3'); // Third table
    
        // Initialize the tab_text with an opening table tag
        ...
        for (var j = 0; j < tab1.rows.length; j++) {
            tab_text += tab1.rows[j].innerHTML + "</tr>";
        }
    
        // Append a separator between tables for clarity
        tab_text += "</table><br><table border='2px'>";
    
        // Append content from the second table
        for (var k = 0; k < tab2.rows.length; k++) {
            tab_text += tab2.rows[k].innerHTML + "</tr>";
        }
    
        // Append a separator between tables for clarity
        tab_text += "</table><br><table border='2px'>";
    
        // Append content from the third table
        for (var l = 0; l < tab3.rows.length; l++) {
            tab_text += tab3.rows[l].innerHTML + "</tr>";
        }
    
        // Close the table
        tab_text += "</table>";
    
        // Clean the HTML of unwanted tags but KEEP <img> tags
        tab_text = tab_text.replace(/<A[^>]*>|<\/A>/g, ""); // Remove anchor tags
        tab_text = tab_text.replace(/<input[^>]*>|<\/input>/gi, ""); // Remove input tags
    
        // Check browser compatibility and export
        ...
        if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
            ...
            txtArea1.focus();
            sa = txtArea1.document.execCommand("SaveAs", true, "quotation.xls");
        } else {
