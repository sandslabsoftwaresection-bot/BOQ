$(document).ready(function(){
   ...
   $(":input:not(:hidden)").each(function (i) { $(this).attr('tabindex', i + 1); });
                ...
                var product_list_table = $('#list_of_product').DataTable({searching: false, paging: false, info: false,"ordering": false});
                var product_all_list_table = $('#list_of_all_product').DataTable({searching: true, paging: true, info: false,"ordering": false});
                var product_item_list_table = $('#list_of_product_details_list').DataTable({searching: false, paging: false, info: false,"ordering": false});
            ...
            //         var prod_name = $('#txt_product_name').val();
            //         if (!/^[a-z0-9\-]+$/i.test(prod_name)) {
        ...
        //         var bank_id= $("#div_bank_select_report_all option:selected").val(); 
        //   // console.log(bank_id);
            
        //     if (bank_id != "0")
        //     {
                
        //     var v_bank_id= $("#div_bank_select_report_all option:selected").val();
        //              var v_bank_name= $("#div_bank_select_report_all  option:selected").text();
        //              var v_bank_collection_from_date = formatDate($("#txt_start_date").val());
        //              var v_bank_collection_to_date = formatDate($("#txt_end_date").val());
        //              var v_collection_status=$("#div_bank_collection_status  option:selected").text();
        //              load_data_to_grid_bank_collection_list_between(v_bank_collection_from_date,v_bank_collection_to_date,v_bank_id,v_collection_status);
        //     window.open("../reports/bank_collection_print.php?v_bank_id="+v_bank_id+"&v_bank_name="+v_bank_name+"&start_date="+v_bank_collection_from_date+"&end_date="+v_bank_collection_to_date+"&collection_status="+v_collection_status,"_blank"); 
       window.open("../reports/list_of_estimation_print.php"); 
            
        //     }
            
        //     else
        //     {
        //       swal("Warning"," Please select a bank name", "warning");   
        //     }
           })
...
//console.log(total_amt);
                    ...
                    //                     							title: "Are you sure?",
                    //                     							text: "Do you want to enter the item into master table?",
                    //                     							icon: 'warning',
                    //                     							dangerMode: true,
                    ...
                    //     function  add_item_master(result) 
                    //   {
                          
                    //       var product_item_id = result; 
                    //      $.post("../controller/product/product_controller.php",{action:'add_item_to_master',v_product_item_id:product_item_id,v_product_code:v_product_code}
                    //                             , function(result,status)
                    //                             {
                    ...
                    //   }  
                                   ...
                                   $("#div_company_select").addClass("disabledbutton");
                                    $("#select_project_name_chosen").addClass("disabledbutton");
                                    $("#div_type_select").addClass("disabledbutton");
                                    //("#select_category_name").val('0');
                                     //$('#select_company_name option[selected]').prop('selected', true);
                                     //$("#div_company_select").addClass("disable-div");
                                   // $('#select_company_name option[value=""]').attr('selected','selected');
                                    // $('#select_company_name').prop('selectedIndex',0);
                   ...
                   $.post("../controller/product/product_controller.php",{action:'list_secondary_details',v_store_id:store_id }
                                , function(result,status)
                                {
                                    ...
                                    wght=obj.data[0].weight;
                                     ...
                                     $("#txt_units_id").val(obj.data[0].sec_unit_id);
                                    
                                     $("#txt_secondary_rate_per_unit").val(obj.data[0].sec_rate_per_unit);
                                     
                                     
                                     $("#txt_primay_rate_per_unit").val(obj.data[0].rate_per_unit);
                                     $("#txt_vat_percentage").val(obj.data[0].vat_percentage);
                                     $("#txt_vat_amount").val(obj.data[0].amount);
                                     $("#txt_vat_prct_amount").val(obj.data[0].vat_amount);
                  ...
                  $.post("../controller/product/product_controller.php",{action:'check_product_code',v_product_code:v_txt_product_code}
                                              ...
                                              if(result=="not exist")
                                              {
                                                ...
                                                else
                                                {
                                                    ...
                                                    // $("#txt_finished_prd_id").val(obj.data[0].finished_product_id);
                                                    // strobj=obj.data[0].finished_product_id;
                                                    //  var obj= jQuery.parseJSON(result);

                                                   
                                                    // $("#txt_product_name").val(obj.data[0].product_name);
                                                    // $("#error_prdt_code").text("Product Code Already Exist ");
   ...
   //product_div_value();
     ...
     // alert(euipment_cost_amnt);
      ...
      //alert(margin_cost_amnt);
     // txt_total_amount_cal=txt_tot_amount;
     v_product_qty=$("#txt_product_qty").val();
     ...
     txt_total_amount_cal=parseFloat(txt_total_amount_calnew,10).toFixed(3);
      //alert(v_labour_cost);
      //txt_total_amount_cal=parseFloat(v_labour_cost)+parseFloat(v_service_cost)+parseFloat(v_euipment_cost)+parseFloat(v_other_cost)+parseFloat(v_margin_cost)+parseFloat(txt_tot_amount);
     //console.log(v_labour_cost);
      $('#cal_value').text(txt_total_amount_cal);
                                ...
                                //  { "data": "company_name" , visible:"false"},
            					 ...
            					 { "data": "labour_cost_type",className: "text-right",
                                  render: function ( data, type, rows ) {
            								...
            								if(rows['labour_cost_type']=='BD')
            								{
            								return '<span style="font-weight:bold;color:#000000">'+rows['product_primary_amt'] * rows['labour_cost']/100 + '</span>'+'   (' +rows['labour_cost']/100 + '%)';
            								}
            								else
            								{
            								return '<span style="font-weight:bold;color:#000000">'+rows['labour_cost_amt']+'</span>'+'   (' +rows['labour_cost'] + ''+ rows['labour_cost_type']+')';
            								}
            				 	 ...
            				 	 render: function ( data, type, rows ) {
            									...
            									if(rows['service_cost_type']=='BD')
            								{
            								return '<span style="font-weight:bold;color:#000000">'+rows['product_primary_amt'] *rows['service_cost']/100 +'</span>'+'   ('+rows['service_cost']/100 +'%)';
            								}
            								else
            								{
            									return '<span style="font-weight:bold;color:#000000">'+rows['service_cost_amt']+'</span>'+'   ('+rows['service_cost'] + ''+ rows['service_cost_type']+')';
            								}
            					 ...
            					 render: function ( data, type, rows ) {
            								...
            								if(rows['equipment_cost_type']=='BD')
            								{
            								   //console.log(rows['product_item_primary_amt'] *rows['equipment_cost']/100);
            								return '<span style="font-weight:bold;color:#000000">'+rows['product_primary_amt'] *rows['equipment_cost']/100 +'</span>'+'   ('+rows['equipment_cost']/100 + '%)';
            								}
            								else
            								{
            								return '<span style="font-weight:bold;color:#000000">'+rows['equipment_cost_amt']+'</span>'+'   ('+rows['equipment_cost'] + ''+ rows['equipment_cost_type']+')';
            								}
            					 ...
            					 render: function ( data, type, rows ) {
            								...
            								if(rows['other_cost_type']=='BD')
            								{
            								return '<span style="font-weight:bold;color:#000000">'+rows['product_primary_amt'] * rows['other_cost']/100+'</span>'+'    ('+rows['other_cost']/100 +  '%)';
            								}
            								else
            								{
            									return '<span style="font-weight:bold;color:#000000">'+rows['other_cost_amt']+'</span>'+'    ('+rows['other_cost'] + ''+ rows['other_cost_type']+ ')';
            								}
            					 ...
            					 render: function ( data, type, rows ) {
            								...
            								if(rows['margin_cost_type']=='BD')
            								{
            								return '<span style="font-weight:bold;color:#000000">'+rows['product_primary_amt'] * rows['margin_cost']/100 +'</span>'+'   ('+rows['margin_cost']/100 + '%)';
            								}
            								else
            								{
            									return '<span style="font-weight:bold;color:#000000">'+rows['margin_cost_amt']+'</span>'+'   ('+rows['margin_cost'] + ''+ rows['margin_cost_type']+')';
            								}
                                      ...
                                      api.column(1, {
                                        ...
                                        if (last !== group) {
                                          $(rows).eq(i).before(
                                            ...
                                            '<tr class="group" style="background-color:#3399ff;font-size: 12px;color:white;font-weight: bold; "><td colspan="15"> Project : ' + group + '</td></tr>'
                                          );
     ...
     $('#list_of_all_product tbody').on('dblclick', 'tr', function(){
                    ...
                    $.post("../controller/product/product_controller.php",{action:'list_item_details_for_edit',v_finished_id:v_edit_product_id}
                            , function(result,status)
                              ...
                              var obj= jQuery.parseJSON(result);
                         var v_company_id=obj.data[0].company_id;     
                            			
                         // $("#select_company_name").val(company_id).trigger("change");
                              $("#txt_pro_id").val(obj.data[0].project_id);   
                    //   $("#select_company_name").find('option').removeAttr("selected");
                    //      $('#select_company_name option').map(function () {
                    //     if ($(this).text() == obj.data[0].company_name) return this;
                    //     }).attr('selected', 'selected');
                        //  $('#select_company_name option').map(function () {
                        // if ($(this).val() == $.trim(v_company_id)) return this;
                        // }).attr('selected', 'selected');
                         $("#select_company_name").val(v_company_id).trigger("change");
                        $("#select_company_name").trigger("chosen:updated");
                    ...
                    $("#div_product_unit_select").find('option').removeAttr("selected");
                   // $("#select_product_unit").val(obj.data[0].product_unit_id).trigger("change");   
                         $('#div_product_unit_select option').map(function () {
                        if ($(this).val() == obj.data[0].product_unit_id) return this;
                         }).attr('selected', 'selected');
