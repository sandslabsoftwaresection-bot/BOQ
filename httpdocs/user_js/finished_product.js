$(document).ready(function(){
    ...
    // $('#div_company_select_material').load('templates/company_combo_estimation.php');
    
    $('#div_company_select').load('templates/company_combo.php', function() {
        $('.chosen_select').chosen();
    });

    $('#div_company_select_material').load('templates/company_combo_estimation.php', function() {
        $('.chosen_select').chosen();
    });
     $('#div_item_select').load('templates/item_combo.php?v_category_id='+0);
   $(":input:not(:hidden)").each(function (i) { $(this).attr('tabindex', i + 1); });
                ...
                var store_list_table = $('#list_of_stores').DataTable( {searching: true, paging: false, info: false,"ordering": false});
        ...
        function clearModalFields() {
            ...
            $("#div_company_select_material").change(function() {
                    ...
                    // $('#div_project_select').load('templates/project_combo.php?company_id='+company_id);
                        ...
                        $.ajax({
                    		...
                    		data: { company_id : company_id } 
                    			...
                    			$("#div_project_select_material").html(data);
                    // 			if(pro_id!=''){
                            
                    //                   // console.log("not null"+pro_id);
                    //                 $('#div_project_select_material option').map(function () {
                    //                 if ($(this).val() == $.trim(pro_id)) return this;
                    //                 }).attr('selected', 'selected');
                    //                 $("#select_project_name_material").trigger("chosen:updated");
                    //                 }
                ...
                $("#div_category_select").change(function() {
                    refreshClicked = 2;
                    var v_category_id = $("#select_category_name").val();  // Get selected category_id
                    // Load items filtered by category_id into div_item_select
                    $('#div_item_select').load('templates/item_combo.php?v_category_id=' + v_category_id, function() {
                        $('#select_item_name').chosen();  // Re-initialize Chosen
                        $('#select_item_name').trigger("chosen:updated");  // Update Chosen
                    });
                });
                
                $("#btn_view_refresh_items").click(function() {
                    refreshClicked = 1;
                    // Load all items
                    $('#div_item_select').load('templates/item_combo.php?v_category_id=0', function() {
                        $('#select_item_name').chosen();  // Re-initialize Chosen
                        $('#select_item_name').trigger("chosen:updated");  // Update Chosen
                        // Optionally reset category dropdown to "Select Category"
                        $('#select_category_name').val(0).trigger("chosen:updated");
                    });
                });
            ...
            //                                 $('#div_category_select').val(obj.data[0].category_id).trigger('change');
            //             $('#select_category_name').val(obj.data[0].category_id);
            //             $('#select_category_name').trigger("chosen:updated");
            $("#div_item_select").change(function() {
                var store_id = $("#select_item_name").val();
                $("#txt_store_id").val(store_id);
                $.post("../controller/product/product_controller.php", {action: 'list_secondary_details', v_store_id: store_id}, function(result, status) {
                    var obj = jQuery.parseJSON(result);
                    console.log("Category_name"+obj.data[0].category_id);
                    if (refreshClicked === 1 || refreshClicked === 0) {
                        $('#select_category_name').val(obj.data[0].category_id);
                        $('#select_category_name').trigger("chosen:updated");
                    }

                    $('#select_item_name').val(store_id);  // Ensure item stays selected
                    $('#select_item_name').trigger("chosen:updated");
                     ...
                     product_list_table = $('#list_of_product').DataTable( {
                            ...
                            "columns": [
                                ...
                                { "data": null },
            					 ...
            					 { "data": "square_meter",className: "text-right", "visible": false},
            					 { "data": "cubic_meter",className: "text-right", "visible": false},
            					  { "data": "vat_percentage",className: "text-right", "visible": false},
     ...
     $('#div_margin_cost').change(function (e) {
        ...
        $('#btn_view_list_of_products1').click(function(){
           ...
           //alert("inside click");
            ...
            $('#div_company_select_material').load('templates/company_combo_estimation.php', function () {
                $('#select_company_material').chosen({
                    width: '100%' // or '300px' if needed
                }).trigger('chosen:updated');
                 // Now clear/reset the project select
                $('#div_project_select_material').html(`
                    <select id="select_project_name_material" class="chosen_select form-control form-control-sm" name="select_project_name_material" style="width: 100%;">
                        <option value="0">Select Project</option>
                    </select>
                `);
            
                // Reinitialize Chosen for the now-reset project select
                $('#select_project_name_material').chosen({
                    width: '100%'
                }).trigger('chosen:updated');
            });
    ...
    $('#list_of_all_product tbody').on('click', 'td button', function(){
       ...
       // alert("inside click");
         ...
         $('#btn_view_estimation_list').on('click',function(){
             
             var primary_cost_project_id=$("#div_project_select_material option:selected").val();
             console.log(primary_cost_project_id);
             load_data_to_grid_all_product_details_list_search(primary_cost_project_id);
             
         })
         
         
          function load_data_to_grid_all_product_details_list_search(project_id)
                 {
                     product_all_list_table.destroy();
                     ...
                     product_all_list_table = $('#list_of_all_product').DataTable( {
                             ...
                             "ajax": {
                                 ...
                                 'url': '../controller/product/product_controller.php',
                                 'data': {
                                    action: 'list_all_finihed_product_details_project',v_project_id:project_id
