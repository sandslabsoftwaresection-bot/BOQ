$(document).ready(function(){
    
    var v_tax_amount,total_amount,vat_added_total_amnt,vat_amnt;
    var str;
    var material_list_report_list_table = $('#material_list_report').DataTable({searching: false, paging: false, info: false,"ordering": false});
    $('#div_company_select_material').load('templates/company_combo.php');
    load_tax_select_box('div_tax_select','select_tax_name');
    function load_tax_select_box(div_name,ctrl_name)
                        
        { 
                          
             $("#"+div_name).load("../controller/store/store_controller.php",{action:'list_tax',v_ctrl_name:ctrl_name},function(result,status){});
        
        } 
    
    $('#div_project_select_material').change(function(){
    
        var material_cost_project_id=$("#div_project_select_material option:selected").val();
        
         $.post("../controller/product/product_controller.php",{action:'find_vat_for_project',v_project_id:material_cost_project_id}
            , function(result,status)
                {
                    console.log('test'+result);
                  var obj= jQuery.parseJSON(result);              
                  var project_vat=obj.data[0].tax_value; 
                  $("#txt_project_vat").val(project_vat);
                });
        
    });
    
    $("#btn_material_list_print").click(function(){
        
        var material_company_id=$("#div_company_select_material option:selected").val();
        var material_company_name=$("#div_company_select_material option:selected").text();
        var material_project_id=$("#div_project_select_material option:selected").val();
        var material_project_name=$("#div_project_select_material option:selected").text();
        v_tax_amount=$("#txt_project_vat").val();
            //window.open("../reports/material_list_print.php?v_company_id="+material_company_id+"&v_company_name="+material_company_name+"&v_project_id="+material_project_id+"&v_project_name="+material_project_name+"&v_tax_amount="+v_tax_amount,"_blank");
            window.open("../reports/pdf/print/material_list_print.php?v_company_id="+material_company_id+"&v_company_name="+material_company_name+"&v_project_id="+material_project_id+"&v_project_name="+material_project_name+"&v_tax_amount="+v_tax_amount,"_blank");  
     })
     
    $('#btn_view_material_list').click(function(){
        
        var material_company_id=$("#div_company_select_material option:selected").val();
        var material_company_name=$("#div_company_select_material option:selected").text();
        var material_project_id=$("#div_project_select_material option:selected").val();
        var material_project_name=$("#div_project_select_material option:selected").text();
        v_tax_amount=$("#txt_project_vat").val();
         $("#vat_prctg").html(v_tax_amount);
        
     $.post("../controller/product/product_controller.php",{action:'find_total_amnt_item',v_company_id:material_company_id,v_project_id:material_project_id}
            , function(result,status)
                {
                    console.log(result);
                  var obj= jQuery.parseJSON(result);              
                  var total_amnt=obj.data[0].item_total_total_amnt; 
                  $("#txt_total_amnt").val(total_amnt);
                  var tot_amount=$("#txt_total_amnt").val();
                  
                  vat_amnt=((parseFloat(tot_amount)*parseFloat(v_tax_amount))/100).toFixed(3);
                 total_amount=parseFloat(tot_amount).toFixed(3);
                vat_added_total_amnt=(parseFloat(total_amount)+((parseFloat(total_amount)*parseFloat(v_tax_amount))/100)).toFixed(3);
                if($.trim(material_company_id)=="0"||$.trim(material_company_name)==""||$.trim(material_project_id)=="select"||$.trim(material_project_name)=="")
                    
            {
                swal("Warning","Please provide all the details ....", "warning");
                
                return false;
            }
        material_list_load_data_to_grid_product_details_list(material_company_id,material_company_name,material_project_id,material_project_name,v_tax_amount,total_amount,vat_added_total_amnt,vat_amnt);
    
                
                
                
                });
        
    });
    
    
    $("#div_company_select_material").change(function() {
                      
                     
            $('#txt_product_company_id_material').val($('option:selected', this).val()) ;
            var company_id=$('option:selected', this).val() ;
                    
            $('#div_project_select_material').load('templates/project_combo.php?company_id='+company_id);
                      
                      
                
    });
         
    
    function material_list_load_data_to_grid_product_details_list(material_company_id,material_company_name,material_project_id,material_project_name,v_tax_amount,total_amount,vat_added_total_amnt,vat_amnt)
                 {
                     material_list_report_list_table.destroy();
                         
                     material_list_report_list_table = $('#material_list_report').DataTable( {
                            
                             "ajax": {
                                 'type': 'POST',
                                 'url': '../controller/product/product_controller.php',
                                 'data': {
                                    action: 'list_product_report_material_list',
                                    v_company_id:material_company_id,
                                    v_project_id:material_project_id
                                 }
                             },
                              "select": {
                                style: 'multi'
                             },
                             "language": {
                                 "zeroRecords": "No records available",
                                 "infoEmpty": "No records available",
                              },
                            "order": [[ 1, "desc" ]],
            				"bPaginate": false,
            				"bLengthChange": false,
            				"bFilter": false,
            				"bInfo": false,
            				"autoWidth": false,
            				 dom: 'Bfrtip',
                            buttons: [
                            {
                                extend: 'excelHtml5',
                                filename:'Material List - '+material_project_name,
                               title: 'MATERIAL LIST',
                               className: 'advisorsExportButton' ,
                                //text:'Export to excel',
                                 text: '<i class="material-icons icon-lg">assignment_returned</i>',
                                messageTop: 'Company Name : '+material_company_name+
                                '  Project Code : '+material_project_id+
                                '  Project Name : '+material_project_name,
                                 messageBottom: 'Total Amount : '+total_amount+
                                '\r\nVAT ('+parseFloat(v_tax_amount).toFixed(2)+'%): '+vat_amnt+
                                '\r\nGrand Total : '+vat_added_total_amnt,
                        //Columns to export
                                exportOptions: {
                                columns: [ 0,1, 2, 3, 4, 5, 6]
                                }
                            },
            //                 {
            //                     extend: 'pdfHtml5',
            //                     className: 'btn-info',
            //                   title: 'MATERIAL LIST',
            //                     text: 'Export to PDF',
            //                     filename:'Material List - '+material_project_name,
            //                     messageTop: 'Company Name : '+material_company_name+
            //                     '\r\nProject Code : '+material_project_id+
            //                     '\r\nProject Name : '+material_project_name,
            //                      messageBottom: 'Total Amount : '+total_amount+
            //                     '\r\nVAT ('+v_tax_amount+'%): '+vat_amnt+
            //                     '\r\nGrand Total : '+vat_added_total_amnt,
            //             //Columns to export
            //                     exportOptions: {
            //                         columns: [ 0,1, 2, 3, 4, 5, 6]
            //                     },
            //                      customize: function(doc) {
            //                      doc.defaultStyle.fontSize = 12; //<-- set fontsize to 16 instead of 10
            //                      doc.styles.title.fontSize = 15;
            //                     doc.styles.title.alignment = 'right';
            //                     doc.styles.tableHeader.color = '#000000';
            //                     doc.styles.tableHeader.fillColor = '#ffffff';
            //                     doc.styles.tableBodyOdd.fillColor = '#ffffff';
            //                     doc.pageMargins = [40,70,40,30];
            //                     doc.content[2].table.widths = ['10%',  '24%', '14%', '14%', 
            //                                               '14%', '10%', '14%'];
            //                      var logo='data:image/png;base64,';
            //                     doc['header']=(function() {
							     //       return {
								    //         columns: [
									   //     {
										  //      image: logo,
										  //      width: 125,
										  //      margin: [40,30]
										  //      //alignment: 'left',
									   //     },
									       
								    //     ],
								    //   // margin: 20
							     //       }
						      //      });
            //                         var objLayout = {};
						      //      objLayout['hLineWidth'] = function(i) { return .5; };
						      //      objLayout['vLineWidth'] = function(i) { return .5; };
						      //      objLayout['hLineColor'] = function(i) { return '#000000'; };
						      //      objLayout['vLineColor'] = function(i) { return '#000000'; };
						           
						      //      doc.content[2].layout = objLayout;
                                
            //                      }
            //                 }
                            ],
                            initComplete: function () {
                                var btns = $('.dt-button');
                                btns.addClass('btn btn-success btn-sm');
                                btns.removeClass('dt-button');

                            },
            				
                            "columns": [
                                  
                                { "data":'finished_product_id', defaultContent: '' ,className: "text-center"},
                                 
                                 
                                 { "data": "item_name" },
                                 { "data": "item_total_qty",className: "text-center"},
                                 { "data": "units",className: "text-center"},
                                  { "data": "rate_per_unit",className: "text-right"},
                                   
                                  
                                //   { "data": "rate_per_unit",className: "text-right",
                                //   render: function ( data, type, rows ) {
                                //     return ((((rows['vat_percentage']*rows['rate_per_unit'])/100)*(rows['item_total_qty'])).toFixed(3)).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
                                //   }		
                                //   },
                                  { "data": "tot_vat_prct_amount",className: "text-right",
                                   render: function ( data, type, rows ) {
                                  return (parseFloat(data).toFixed(3)).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
                                  }		
                                  },
                                    { "data": "tot_item_total_amnt",className: "text-right",
                                   render: function ( data, type, rows ) {
                                  return (parseFloat(data).toFixed(3)).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
                                  }		
                                  }
                              
                                //   { "data": "item_total_amnt", className: "text-right",
                                  
                                //   render: function ( data, type, rows ) {
                                     
                                //       var item_total=(rows['vat_percentage']*rows['rate_per_unit']*rows['item_total_qty'])/100;
                                //       return ((parseFloat(item_total)+parseFloat(rows['item_total_amnt'])).toFixed(3)).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
                                   
                                //     }
                                //   } 
                                  
                                  
            					
            				 	
                             ],
                             "footerCallback": function ( row, data, start, end, display ) {
                                            var api = this.api(), data;
                                 
                                        
                                                
                                                
                                                // $( api.column( 5 ).footer() ).html(
                                                //     $.fn.dataTable.render.number(',', '.', 3, 'VAT ('+v_tax_amount+'%): ').display( vat_amnt )
                                                // );
                                                // $( api.column( 0 ).footer() ).html(
                                                //     $.fn.dataTable.render.number(',', '.', 3, 'Total:  ').display( total_amount )
                                                // );
                                                $('tr:eq(1) th:eq(1)', api.table().footer()).html(
                                                    $.fn.dataTable.render.number(',', '.', 3, '').display( total_amount )
                                                );
                                                
                                                $('tr:eq(2) th:eq(1)', api.table().footer()).html(
                                                    $.fn.dataTable.render.number(',', '.', 3, '').display( vat_amnt )
                                                );
                                                $('tr:eq(3) th:eq(1)', api.table().footer()).html(
                                                    $.fn.dataTable.render.number(',', '.', 3, '').display( vat_added_total_amnt )
                                                );
                                               
                                },
                             
                             pageLength: 1000,
            				 searching: false,
                            responsive: true,
                            
                             "aoColumnDefs": [
            					{ "bSortable": false, "aTargets": [  0,1,2,3,4,5],
            					
            					     "mRender": function (data, type, full) {
                                     var formmatedvalue=data.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")
                                         return formmatedvalue;
                                    }
            					}, 
            					
            				],
            				
                           	drawCallback: function() {
                              var hasRows = this.api().rows({ filter: 'applied' }).data().length > 0;
                              $('.buttons-excel')[0].style.visibility = hasRows ? 'visible' : 'hidden'
                            }
                                    
                     });  
                
                 }
                 
                material_list_report_list_table.on( 'order.dt search.dt', function () {
                material_list_report_list_table.column(0, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
                cell.innerHTML = i + 1;
                material_list_report_list_table.cell(cell).invalidate('dom'); 
                } );
                } ).draw();
});